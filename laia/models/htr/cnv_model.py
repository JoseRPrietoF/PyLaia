"""
Code from https://github.com/IntuitionMachines/OrigamiNet
"""
import torch
import torch.nn as nn
import torch.nn.functional as F
# from laia.models import htr
from laia.models.htr.chk import checkpoint_sequential_step, checkpoint
import math
import numpy as np
...
class GateBlock(nn.Module):
  #GradCheck=gin.REQUIRED
    def __init__(self, ifsz, ofsz, gt = True, ksz = 3, GradCheck=1):
        super().__init__()
...
class InitBlock(nn.Module):
    def __init__(self, fup, n_channels=3):
        super().__init__()
...
class OrigamiNet(nn.Module):
    def __init__(self, num_input_channels, num_output_labels, wmul, lreszs, lszs, nlyrs, fup, GradCheck, reduceAxis=3, **kwargs):
        super().__init__()
        self.lreszs = lreszs
        self.Initsq = InitBlock(fup, n_channels=num_input_channels)
        
        layers = []
        isz = fup + num_input_channels
        osz = isz
